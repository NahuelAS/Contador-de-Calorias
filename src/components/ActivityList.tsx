...
export default function ActivityList({activities, dispatch}: ActivityListProps) {
    ...
    return (
        <>
            ...
            {isEmptyActivities ? 
                <p className="text-center my-5 text-sm text-gray-400">No Hay Actividades aún...</p> :
                ...
                <div key={activity.id} className="px-5 py-10  bg-white mt-5 flex justify-between shadow">
                    ...
                    <div className="space-y-2 relative">
                        <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold 
                            ${activity.category === 1 ? 'bg-green-500' : 'bg-yellow-500'
                        }`}>
                            {categoryName(+activity.category)}
                        </p>
                        <p className="text-2xl font-bold pt-5">{activity.name}</p>
                        <p className="font-black text-4xl text-neutral-600 ">
                            {activity.calories} {' '}
                            <span>Calorias</span>
                        </p>
                    </div>
